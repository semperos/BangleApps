// Clock with large digits using the "Anton" bold font, customized by Daniel Gregoire (dlg)

const SETTINGSFILE = "dlgantonclk.json";
...
function draw() {
  ...
  if (secondsScreen) {
    ...
    if (calWeek || weekDay) {
      ...
      var finalDowStr = '';
      for (var i = 0; i < dowcwStr.length; i++) {
        if (dowcwStr[i] === 'A' ||
            dowcwStr[i] === 'E' ||
            dowcwStr[i] === 'I' ||
            dowcwStr[i] === 'O' ||
          ...
          finalDowStr += dowcwStr[i];
        }
      }
      g.drawString(finalDowStr, x, y + (vectorFont ? 26 : 16));
    }
  }
